...
class DeepSets(nn.Module):
    def __init__(self, param, latent_dim, epochs, lr, wd, dropout=0.5, bs=32, n_classes=2, use_weight=False):
        super().__init__()
    ...
    def fit(self, parameters, labels):
        ...
        weight = torch.tensor([0.9,0.1]).to(self.device)
        criterion = torch.nn.CrossEntropyLoss() if not self.use_weight else torch.nn.CrossEntropyLoss(weight=weight)
    ...
    def predict(self, parameters):
        ...
        if self.n_classes > 1:
            for X in loader:
                predictions.append(self.forward(X).detach().cpu())
        else:
            for X in loader:
